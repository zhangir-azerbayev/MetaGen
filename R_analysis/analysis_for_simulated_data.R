...
data$Euclidean.distance.between.avg_Rs.and.gt_R.MH <- data$Euclidean.distance.between.avg_Rs.and.gt_R.MH %>%
  lapply(function(x){gsub(pattern = "Any[", replacement="",x, fixed = TRUE)})
data$Euclidean.distance.between.avg_Rs.and.gt_R.MH <- data$Euclidean.distance.between.avg_Rs.and.gt_R.MH %>%
...
data$avg_Vs_binned.for.unique_realities.MH <- data$avg_Vs_binned.for.unique_realities.MH %>%
  lapply(function(x){gsub(pattern = "Array{Float64,N} where N[[", replacement="",x, fixed = TRUE)})
data$avg_Vs_binned.for.unique_realities.MH <- data$avg_Vs_binned.for.unique_realities.MH %>%
  lapply(function(x){gsub(pattern = "]", replacement="",x, fixed = TRUE)})
