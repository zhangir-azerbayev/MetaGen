...
gt_miss2 <- rep(0.99, n_percepts)
gt_fa5 <- rep(0.3, n_percepts)
...
df_M = gather(toPlot, "M_line", "value", c(miss2, gt_miss2, chance_miss2)) %>%
  group_by(M_line)
...
df_FA = gather(toPlot, "FA_line", "value", c(fa5, gt_fa5, chance_fa5)) %>%
  group_by(FA_line)
