...
function (args)
    if on_right_side(camera_params, object) > 0
        locate(camera_params, permanent_camera_params, object)
    end
end
...
function locate(camera_params::Camera_Params,
    ...
    angle_from_vertical = rad2deg(get_angle(a_vertical, b_vertical, c_vertical, object.x, object.y, object.z))
    angle_from_horizontal = rad2deg(get_angle(a_horizontal, b_horizontal, c_horizontal, object.x, object.y, object.z))
...
function get_abc_plane(p1::Coordinate, p2::Coordinate, p3::Coordinate)
    ...
    D = det([p1.x p1.y p1.z; p2.x p2.y p2.z; p3.x p3.y p3.z])

    if D==0
        print("crap! determinant D=0")
    end
    #implicitly going to say d=-1 to obtain solution set
    a = det([1 1 1; p2.x p2.y p2.z; p3.x p3.y p3.z])/D
    b = det([p1.x p1.y p1.z; 1 1 1; p3.x p3.y p3.z])/D
    c = det([p1.x p1.y p1.z; p2.x p2.y p2.z; 1 1 1])/D
