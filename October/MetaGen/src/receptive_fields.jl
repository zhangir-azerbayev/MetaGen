...
function get_rfs(rec_field::Receptive_Field, imaginary::Vector{Detection2D}, real::Vector{Detection2D}, params::Video_Params)
    ...
    imagined_objects_2D = to_elements(imaginary, fas)

    #for real objects
    hits = params.v[:,2]
    #hitses = fill(hits, length(real))
    #real_objects_2D = map(to_element, imaginary, 1 .- hitses)
    real_objects_2D = to_elements(imaginary, hits)
...
function to_elements(objects::Vector{Detection2D}, ps::Vector{Float64})
    sd_x = 1.
    sd_y = 1.
    cov = [sd_x 0.; 0. sd_y;]
    ...
    for i = 1:n
        ...
        y = objects[n][2]
        cat = objects[n][3]
        p = ps[cat] #get the fa for this category
        objects_2D[i] = BernoulliElement{Detection2D}(p, object_distribution_image, ([x, y], cov, cat))
    end

    #if vector was empty, put in a BernoulliElement with p=0
    if n==0
        objects_2D = Vector{BernoulliElement{Detection2D}}(undef, 1)
        objects_2D[1] = BernoulliElement{Detection2D}(0, object_distribution_image, ([-1000, -1000], cov, 0))
    end
