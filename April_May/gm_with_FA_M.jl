...
@gen function build_percept(R, V::Matrix{Float64}, fragmentation_lambda::Float64, fragmentation_max::Float64, hallucination_max::Float64, possible_objects)
	...
	for j = 1:length(possible_objects)
		...
		# if possible_object in R
		# 	#detection
		# 	detection = @trace(bernoulli(1-M), :detection  => j)
		# 	hallucination = 0
		# else
		# 	hallucination = @trace(bernoulli(FA), :hallucination  => j)
		# 	detection = 0
		# end
		#
		# sum_count = detection + hallucination #integer, possibly 0
		# # sum_count = convert(Float64, sum_count)
		# # lam = max(sum_count, 1.0E-4) #need lambda to be non-zero
		# # visual_count = @trace(trunc_poisson(lam, 0.0, 10.0), (:visual_count => j)) #problem for particle filter
		#
		# #visual_count = @trace(trunc_poisson(lam, sum_count-1.0, sum_count), (:visual_count => j)) #problem for particle filter when sum_count is an RV
		# #visual_count = @trace(trunc_poisson(5.0, 4.0, 5.0), (:visual_count => j)) #no problem for particle filter
		# #visual_count = @trace(trunc_poisson(1.0E-4, -1.0, 0.0), (:visual_count => j)) #no problem for particle filter
		# #visual_count = @trace(trunc_poisson(1.0, 0.0, 1.0), (:visual_count => j)) #no problem for particle filter
		#
		# n = 15 #some maximum on how many things can be detected
		# highest_prob = 0.99 #probability of sum_count
		# prob = fill((1-highest_prob)/(n-1), n)
		# prob[sum_count + 1] = highest_prob #prob[1] will be for visual_count=0, so sum_count+1 will be for sum_count
		#
		# visual_count = @trace(categorical(prob), :visual_count  => j)
		#
		# for i = 1:(visual_count-1)
		# 	push!(perceived_frame, possible_object)
		# end
		#
		#
		...
		# 	detection_count = @trace(bernoulli(1-M), :detection_count  => j)
		...
		# 	hallucination_count = @trace(bernoulli(FA), (:hallucination_count => j))
		...
		#
		# sum_count = detection_count + hallucination_count #integer, possibly 0
		# sum_count = convert(Float64, sum_count)
		# lam = max(sum_count, 1.0E-4) #need lambda to be non-zero
		# visual_count = @trace(trunc_poisson(lam, sum_count-1.0, sum_count), (:visual_count => j))
		# #visual_count = @trace(trunc_poisson(5.0, 4.0, 5.0), (:visual_count => j))
		#
		#
		#
