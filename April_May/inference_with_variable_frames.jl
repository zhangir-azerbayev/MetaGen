...
#obs is a choicemap to add the observations to
#gt_choices are the ground truth choices as a choicemap
function set_ambiguous_observation(obs, ambiguous_percept, p, n_frames)
    for f = 1:n_frames
        for j = 1:length(possible_objects)
            possible_object = possible_objects[j]
            possible_object in  ambiguous_percept[f] ? visual_count = true :  visual_count = false
...
function particle_filter(num_particles::Int, n_percepts, n_frames, ambiguous_percept, gt_choices, num_samples::Int, mean_Beta::Float64, lesion::Bool)
    ...
    lesion ? set_initial_observations(init_obs, gt_choices, p, n_frames_in_this_percept, mean_Beta) : set_observations(init_obs, gt_choices, p, n_frames_in_this_percept)
	...
	state = Gen.initialize_particle_filter(gm, (possible_objects, 1, n_frames_in_this_percept), init_obs, num_particles)
    ...
    n_frames_in_this_percept = length(ambiguous_percept)
    obs = Gen.choicemap()
    set_ambiguous_observation(obs, ambiguous_percept, p, n_frames_in_this_percept)

    #attaching the number of frames in the ambiguous percept to the n_frames vector for the pf step
    ambig_n_frames = append!(n_frames, n_frames_in_this_percept)
    Gen.particle_filter_step!(state, (possible_objects, p, ambig_n_frames), (UnknownChange(),), obs)

    # return a sample of unweighted traces from the weighted collection
    tr_ambiguous_percept = Gen.sample_unweighted_traces(state, num_samples)

    print_Vs_and_Rs_to_file(tr_ambiguous_percept, num_samples, possible_objects)
